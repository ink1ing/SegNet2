...
def save_checkpoint(state, is_best_loss, is_best_f1, epoch, output_dir):
    """
    保存模型检查点，包括当前状态、最佳损失模型和最佳F1模型
    
    参数:
        state: 包含模型状态等信息的字典
        is_best_loss: 是否是最佳损失模型
        is_best_f1: 是否是最佳F1分数模型
        epoch: 当前训练轮次
        output_dir: 输出目录
    """
    # 确保输出目录存在
    ...
    # 保存当前轮次模型
    filepath = os.path.join(output_dir, f'checkpoint_epoch_{epoch}.pth')
    torch.save(state, filepath)
    
    # 如果是最佳损失模型，单独保存
    if is_best_loss:
        best_loss_path = os.path.join(output_dir, 'best_loss_model.pth')
        torch.save(state['model_state_dict'], best_loss_path)
        print(f"保存最佳损失模型到 {best_loss_path}")
    
    # 如果是最佳F1模型，单独保存
    if is_best_f1:
        best_f1_path = os.path.join(output_dir, 'best_model.pth')
        torch.save(state['model_state_dict'], best_f1_path)
        print(f"保存最佳F1模型到 {best_f1_path}")
        
    # 保存当前最新模型（无论是否最佳）
    last_model_path = os.path.join(output_dir, 'last_model.pth')
    torch.save(state['model_state_dict'], last_model_path)
